...
def upgrade():
    ...
    op.add_column(u'account', sa.Column('account_type_id', sa.Integer(),
                                        nullable=False, server_default=str(account_type.id)))
    op.add_column(u'account', sa.Column(
        'identifier', sa.String(length=256), nullable=True))
    op.create_foreign_key('account_account_type_id_fkey',
                          'account', 'account_type', ['account_type_id'], ['id'])
...
def downgrade():
    ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint('account_account_type_id_fkey',
