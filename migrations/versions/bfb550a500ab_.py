...
class Item(Base):
    ...
    tech_id = sa.Column(sa.Integer, sa.ForeignKey(
        "technology.id"), nullable=False)
...
class ItemRevision(Base):
    ...
    date_created = sa.Column(
        sa.DateTime(), default=datetime.datetime.utcnow, nullable=False, index=True)
...
def upgrade():
    ...
    op.add_column('item', sa.Column(
        'latest_revision_complete_hash', sa.String(32), nullable=True))
    op.add_column('item', sa.Column(
        'latest_revision_durable_hash', sa.String(32), nullable=True))
    op.create_index('ix_item_arn', 'item', ['arn'], unique=True)
    op.create_index('ix_item_name', 'item', ['name'], unique=False)
    op.create_index('ix_item_latest_revision_complete_hash', 'item', [
                    'latest_revision_complete_hash'], unique=False)
    op.create_index('ix_item_latest_revision_durable_hash', 'item', [
                    'latest_revision_durable_hash'], unique=False)
    ...
    for item in query.all():
        ...
        complete_hash, durable_hash = hash_item(
            revision.config, ephemeral_paths)
