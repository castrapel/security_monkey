...
def upgrade():
    ...
    op.add_column('auditorsettings', sa.Column(
        'issue_text', sa.String(length=512), nullable=True))
    op.drop_column('auditorsettings', 'notes')
    op.drop_column('auditorsettings', 'issue')
    op.add_column('itemaudit', sa.Column(
        'auditor_setting_id', sa.Integer(), nullable=True))
    op.create_foreign_key(None, 'itemaudit', 'auditorsettings', [
                          'auditor_setting_id'], ['id'])
...
def downgrade():
    ...
    op.add_column('auditorsettings', sa.Column('issue', sa.VARCHAR(
        length=512), autoincrement=False, nullable=False))
    op.add_column('auditorsettings', sa.Column(
        'notes', sa.VARCHAR(length=512), autoincrement=False, nullable=True))
