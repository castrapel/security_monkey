...
def update_custom_value(name, value, session, account_id):
    ...
    if cv.count():
        ...
        cv = AccountTypeCustomValues(
            name=name, value=value, account_id=account_id)
...
def upgrade():
    ...
    for account in accounts:
        ...
        update_custom_value('role_name', account.role_name,
                            session, account.id)
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('account', sa.Column('s3_name', sa.VARCHAR(
        length=64), autoincrement=False, nullable=True))
    op.add_column('account', sa.Column('number', sa.VARCHAR(
        length=12), autoincrement=False, nullable=True))
    op.add_column('account', sa.Column('role_name', sa.VARCHAR(
