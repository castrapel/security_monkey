...
abstract_classes = set(
    ['Watcher', 'CloudAuxWatcher', 'CloudAuxBatchedWatcher'])

if not app.config.get("DONT_IGNORE_BOTO_VERBOSE_LOGGERS"):
    logging.getLogger('botocore.vendored.requests.packages.urllib3').setLevel(
        logging.WARNING)
...
class WatcherType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__name__ not in abstract_classes and cls.index:
            app.logger.debug("Registering watcher {} {}.{}".format(
...
class Watcher(object):
    ...
    def __init__(self, accounts=None, debug=False):
        ...
        if not accounts:
            ...
            accounts = Account.query.filter(Account.third_party == False).filter(
                Account.active == True).filter(Account.name.in_(accounts)).all()
        if not accounts:
        ...
        # Keeps track of the batch we are on -- can be used for retry logic
        self.batch_counter = 0
        # Tuple that holds the current account and account index we are on.
    ...
    def prep_for_slurp(self):
        ...
        query = query.join(
            (Technology, Technology.id == IgnoreListEntry.tech_id))
    ...
    def prep_for_batch_slurp(self):
        ...
        if not self.current_account:
            ...
            technology = Technology.query.filter(
                Technology.name == self.index).first()
            if not technology:
                technology = Technology(name=self.index)
                db.session.add(technology)
                db.session.commit()
                app.logger.info(
                    "Technology: {} did not exist... created it...".format(self.index))

            self.technology = technology
        else:
            index = self.current_account[1] + 1

        self.current_account = (Account.query.filter(
            Account.name == self.accounts[index]).one(), index)

        # We will not be using CloudAux's iter_account_region for multi-account -- we want
        # to have per-account level of batching
        # Reset the total list for a new account to run against.
        self.total_list = []
    ...
    def slurp_exception(self, location=None, exception=None, exception_map={}, source="watcher"):
        ...
        if location in exception_map:
            app.logger.debug(
                "Exception map already has location {}. This should not happen.".format(location))
        exception_map[location] = exception
        app.logger.debug("Adding {} to the exceptions list. Exception was: {}".format(
    ...
    def location_in_exception_map(self, item_location, exception_map={}):
        ...
        if item_location in exception_map:
            app.logger.debug("Skipping {} due to an item-level exception {}.".format(
                item_location, exception_map[item_location]))
            return True

        # (index, account, region)
        if item_location[0:3] in exception_map:
            app.logger.debug("Skipping {} due to a region-level exception {}.".format(
                item_location, exception_map[item_location[0:3]]))
            return True

        # (index, account)
        if item_location[0:2] in exception_map:
            app.logger.debug("Skipping {} due to an account-level exception {}.".format(
                item_location, exception_map[item_location[0:2]]))
            return True

        # (index)
        if item_location[0:1] in exception_map:
            app.logger.debug("Skipping {} due to a technology-level exception {}.".format(
                item_location, exception_map[item_location[0:1]]))
            return True
    ...
    def find_deleted(self, previous=[], current=[], exception_map={}):
        ...
        item_locations = [item_location for item_location in item_locations if not self.location_in_exception_map(
            item_location, exception_map)]
        list_deleted_items = [prev_map[item] for item in item_locations]

        for item in list_deleted_items:
            deleted_change_item = ChangeItem.from_items(
                old_item=item, new_item=None, source_watcher=self)
            app.logger.debug("%s: %s/%s/%s deleted" %
                             (self.i_am_singular, item.account, item.region, item.name))
    ...
    def find_new(self, previous=[], current=[]):
        ...
        for item in list_new_items:
            new_change_item = ChangeItem.from_items(
                old_item=None, new_item=item, source_watcher=self)
            self.created_items.append(new_change_item)
            app.logger.debug("%s: %s/%s/%s created" %
                             (self.i_am_singular, item.account, item.region, item.name))
    ...
    def find_modified(self, previous=[], current=[], exception_map={}):
        ...
        for location in item_locations:
            ...
            if not sub_dict(prev_item.config) == sub_dict(curr_item.config):
                eph_change_item = ChangeItem.from_items(
                    old_item=prev_item, new_item=curr_item, source_watcher=self)
            ...
            if self.ephemerals_skipped():
                ...
                if eph_change_item:
                    ...
                    app.logger.debug("%s: ephemeral changes in item %s/%s/%s" % (self.i_am_singular,
                                                                                 eph_change_item.account,
                                                                                 eph_change_item.region,
                                                                                 eph_change_item.name))
                if dur_change_item:
                    self.changed_items.append(dur_change_item)
                    app.logger.debug("%s: durable changes in item %s/%s/%s" % (self.i_am_singular,
                                                                               dur_change_item.account,
                                                                               dur_change_item.region,
                                                                               dur_change_item.name))

            elif eph_change_item is not None:
                # store all changes, handle them all equally
                self.changed_items.append(eph_change_item)
                app.logger.debug("%s: changes in item %s/%s/%s" % (self.i_am_singular,
                                                                   eph_change_item.account, eph_change_item.region,
                                                                   eph_change_item.name))
    ...
    def find_changes(self, current=None, exception_map=None):
        ...
        if self.batched_size > 0:
            ...
            self.find_deleted(previous=prev, current=current,
                              exception_map=exception_map)
            self.find_new(previous=prev, current=current)
            self.find_modified(previous=prev, current=current,
                               exception_map=exception_map)
    ...
    def find_changes_batch(self, items, exception_map):
        ...
        for item in items:
            complete_hash, durable_hash = hash_item(
                item.config, self.ephemeral_paths)
            ...
            if created_changed == 'created':
                self.created_items.append(ChangeItem.from_items(
                    old_item=None, new_item=item, source_watcher=self))
    ...
    def find_deleted_batch(self, exception_map):
        ...
        existing_arns = [item["Arn"]
                         for item in self.total_list if item.get("Arn")]
        deleted_items = inactivate_old_revisions(
            self, existing_arns, self.current_account[0], self.technology)
    ...
    def save(self):
        ...
        """
        app.logger.info("{} deleted {} in {}".format(
            len(self.deleted_items), self.i_am_plural, self.accounts))
        app.logger.info("{} created {} in {}".format(
            len(self.created_items), self.i_am_plural, self.accounts))
        for item in self.created_items + self.deleted_items:
            item.save(self.datastore)

        if self.ephemerals_skipped():
            changed_locations = [item.location()
                                 for item in self.changed_items]

            new_item_revisions = [
                item for item in self.ephemeral_items if item.location() in changed_locations]
            app.logger.info("{} changed {} in {}".format(
                len(new_item_revisions), self.i_am_plural, self.accounts))
            for item in new_item_revisions:
                item.save(self.datastore)

            edit_item_revisions = [
                item for item in self.ephemeral_items if item.location() not in changed_locations]
            app.logger.info("{} ephemerally changed {} in {}".format(
                len(edit_item_revisions), self.i_am_plural, self.accounts))
            for item in edit_item_revisions:
                item.save(self.datastore, ephemeral=True)
        else:
            app.logger.info("{} changed {} in {}".format(
                len(self.changed_items), self.i_am_plural, self.accounts))
            for item in self.changed_items:
                item.save(self.datastore)
    ...
    def get_interval(self):
        ...
        config = WatcherConfig.query.filter(
            WatcherConfig.index == self.index).first()
        if config:
...
class ChangeItem(object):
    ...
    def save(self, datastore, ephemeral=False):
        ...
        app.logger.debug("Saving {}/{}/{}/{}\n\t{}".format(self.index,
                                                           self.account, self.region, self.name, self.new_config))
...
def ensure_item_has_latest_revision_id(item):
    ...
    if not item.latest_revision_id:
        ...
        if not current_revision:
            ...
            item.latest_revision_complete_hash = ds.hash_config(
                current_revision.config)
            item.latest_revision_durable_hash = ds.durable_hash(
                current_revision.config, ephemeral_paths)
